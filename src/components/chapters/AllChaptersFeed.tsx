...
const AllChaptersFeed: DCI = () => {
    // Aggregate all news and events
    const allNews = Object.entries(chapterContent).flatMap(([slug, content]) =>
        content.news.map(item => ({ ...item, chapterSlug: slug, chapterName: chapters.find(c => c.slug === slug)?.name }))
    ).slice(0, 3);
        ...
        <div className={feedContainer}>
            <div className={section}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <TypeHeading level={2}>Latest Chapter News</TypeHeading>
                    <Link to="/chapters/news" style={{ textDecoration: 'underline' }}>View All</Link>
                </div>
                <div className={grid}>
                    {allNews.map((item, i) => (
                        <Link key={i} to={`/chapters/${item.chapterSlug}/news`} className={card}>
            ...
            <div className={section}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <TypeHeading level={2}>Upcoming Chapter Events</TypeHeading>
                    <Link to="/chapters/events" style={{ textDecoration: 'underline' }}>View All</Link>
                </div>
                <div className={grid}>
...
export default AllChaptersFeed;
